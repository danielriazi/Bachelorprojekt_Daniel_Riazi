...
for(i in seq_along(df)) {
  tmp <- data.frame(
    x = x,
    Density = dt(x + qt(0.025, df = df[i]) - q, df = df[i]),
    df = factor(df[i])
  )
  data <- rbind(data, tmp)
}
...
# Fixed VaR(2.5%) illustration i teori
...
ESblue <- -ES(100, 100, 0.025)
ESred <- -ES(3, 100, 0.025)

# Plot
ggplot(data, aes(x = x, y = Density, color = df)) +
  geom_line(linewidth = 1) +
  geom_vline(xintercept = q, linetype = "solid", color = "black", linewidth = 1) +
  geom_vline(xintercept = ESblue, linetype = "dashed", color = "blue", linewidth = 1) +
  geom_vline(xintercept = ESred, linetype = "dashed", color = "red", linewidth = 1) +
  scale_x_continuous(breaks = seq(-6, 6, by = 2)) +
  labs(
    title = "Student t-fordeling med fixed VaR(2,5%)",
    x = "x",
    y = "Tæthed",
    color = "Frihedsgrader"
  ) +
  scale_color_manual(values = c("100" = "blue", "3" = "red")) +
  theme_minimal()
```
...
for(i in seq_along(df)) {
  tmp <- data.frame(
    x = x,
    Density = dstd(x + qstd(0.025, 0, 1, nu = df[i]) - q, 0, 1, nu = df[i]),
    df = factor(df[i])
  )
  data <- rbind(data, tmp)
}
...
pdf <- ggplot(pdf_data, aes(x = x, y = y, color = df)) +
  geom_line(linewidth = 1) +
  labs(title = "", x = "", y = "Tæthed", color = "Df") + scale_y_continuous(breaks = seq(0, 4, by = 0.5), minor_breaks = seq(0, 4, by = 0.5)) + scale_x_continuous(breaks = seq(-4, 0, by = 1), minor_breaks = seq(-4, 0, by = 1), limits = c(-4.5, 1) ) + scale_color_manual(values = c("10" = "blue", "5" = "red")) +
