...
Z3 <- function(Xt, sigma, alpha, df){
  ...
  EVanalytical <- -(T/integer)*(integrate(function(x) {pbeta(1 - x, T - integer, integer)*qstd(x, 0, 1, df)}, 0, 1)$value*sigma)
  
  Ut <- pstd(Xt, 0, sigma, df)
  qt <- qstd(Ut, 0, 1, df)
...
ggplot(df_plot, aes(x = t)) +
  ...
  # VaR (rød linje, stiplet)
  geom_line(aes(y = VaR, color = "VaR"), linetype = "solid") +
  
  # ES (blå linje, stiplet)
  geom_line(aes(y = ES, color = "ES"), linetype = "solid") +
  
  # Punkter for overskridelser (samme farve som VaR, men punktmarkering)
  geom_point(
  ...
  # Definer farver og legendetekst
  scale_color_manual(
    name = "Størrelser",  # Titel på legend
    values = c(
      "Afkast"               = "black",
      "VaR"                  = "red",
      "ES"                   = "blue",
      "Overskridelse af VaR" = "red"
    )
  ) +
  ...
  # Eventuelt et 'theme', f.eks. minimal
  theme_minimal()
...
geom_vline(xintercept = 3, linetype = "solid", color = "darkgrey") +
  ...
  labs(
    x = "ARCH parameter",
    y = "Power",
    title = "Ændre reaktionen til de kvadrerede afkast",
    color = "Teststatistik",
    shape = "Teststatistik"
  ) + theme_minimal() + theme(panel.grid.major.x = element_blank(), panel.grid.minor.x = element_blank())
...
Xt <- path_sim1$Xt
sigmaH1 <- as.numeric(sqrt(path_sim1$sigma2))
muH1 <- as.numeric(path_sim1$mu)

path_values <- garch_path_h1(T, burnin, omega, alpha0, beta1, X)
sigmaH0 <- as.numeric(sqrt(path_values$sigma2))
muH0 <- as.numeric(path_values$mu)
